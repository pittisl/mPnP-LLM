...
class MLM_RGB(torch.nn.Module):
    ...
    def __init__(
        ...
        self.llm_num_hidden_layers = self.llm_backbone.config.num_hidden_layers
        ...
        self.rgb_key_aligner = nn.Sequential(
            ...
            # nn.Linear(self.rgb_embed_dim, self.llm_embed_dim, bias=True),
        )
    ...
    def forward(
        ...
        mm_key_values = torch.stack([mm_key, mm_value], dim=0) # -> [2, batch, num_heads, num_vit_blocks, head_dim]
        
        # Duplicate num_llm_blocks times -> [num_llm_blocks, 2, batch, num_heads, num_vit_blocks, head_dim]
        mm_key_values = mm_key_values.repeat(self.backward_depth, 1, 1, 1, 1, 1)
        ...
        mm_key_values = mm_key_values * continuous_weights.view(-1, 1, 1, 1, 1, 1)
        mm_key_values_trainable = [
            mm_key_values[i] for i in range(self.backward_depth)
        ]
        mm_key_values_untrainable = [
            torch.zeros_like(mm_key_values[0]) for i in range(self.llm_num_hidden_layers - self.backward_depth)
        ]
    ...
    def generate(
        ...
        mm_key_values = torch.stack([mm_key, mm_value], dim=0) # -> [2, batch, num_heads, num_vit_blocks, head_dim]
        mm_key_values = torch.repeat_interleave(mm_key_values, num_beams, dim=1) # -> [2, batch*num_beams, num_heads, num_vit_blocks, head_dim]

        continuous_weights = torch.sigmoid(self.linker_weights / self.linker_temperature)
        mm_key_values_trainable = [
            mm_key_values * continuous_weights[i] for i in range(self.backward_depth)
        ]
        mm_key_values_untrainable = [
            torch.zeros_like(mm_key_values) for _ in range(self.llm_num_hidden_layers - self.backward_depth)
        ]
        mm_key_values = mm_key_values_untrainable + mm_key_values_trainable
    ...
    def print_flops(self):
        ...
        L = self.llm_backbone.config.num_hidden_layers
...
class MLM_LIDAR(torch.nn.Module):
    ...
    def __init__(
        ...
        self.lidar_key_aligner = nn.Sequential(
            ...
            # nn.Linear(self.lidar_embed_dim, self.llm_embed_dim, bias=True),
        )
