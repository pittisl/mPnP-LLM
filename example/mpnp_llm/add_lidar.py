...
from custom_trainer import AddLiDARTrainer
...
llm_name = "facebook/opt-1.3b" # ["facebook/opt-1.3b", "bigscience/bloomz-1b1"]
llm_type = "opt"
...
if llm_backbone.config.model_type == "opt":
    rgb_lidar_llm = mpnp_opt.MLM_RGB_LIDAR(
    ...
    rgb_lidar_llm = mpnp_bloom.MLM_RGB_LIDAR(
        rgb_encoder=rgb_encoder,
        lidar_encoder=lidar_encoder,
        llm_backbone=llm_backbone,
        max_input_length=max_input_length,
...
train_loader = nuqa.load_nuqa(
    data_path="nuqamini/night_80dimgaussian7/train",
    llm_name=llm_name,
    rgb_processor=rgb_processor,
    text_tokenizer=text_tokenizer,
    attention_mask_expand=4*6+4,
    max_input_length=max_input_length,
...
val_loader = nuqa.load_nuqa(
    data_path="nuqamini/night_80dimgaussian7/validation",
    llm_name=llm_name,
    rgb_processor=rgb_processor,
    text_tokenizer=text_tokenizer,
    attention_mask_expand=4*6+4,
